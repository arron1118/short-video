...
function checkPhpVersion($version): bool
{
    $php_version = explode('-', PHP_VERSION);
    return strnatcasecmp($php_version[0], $version) >= 0;
}

function checkConnect(): bool
{
    try {
...
function getAppConfig($admin): string
{
    return <<<EOT
<?php
// +----------------------------------------------------------------------
...
function getDatabaseConfig($data): string
{
    return <<<EOT
<?php
use think\\facade\Env;
...
<body>
<h1><img src="static/common/images/logo-1.png" alt=""></h1>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <input class="layui-input" id="admin_url" name="admin_url" autocomplete="off" lay-verify="required"
                           lay-reqtext="请输入后台的地址" placeholder="为了后台安全，不建议将后台路径设置为admin"
                           value="admin">
                    <span class="tips">后台登录地址： <?php echo $currentHost; ?><span
                                id="admin_name">admin</span></span>
                </div>
            </div>
